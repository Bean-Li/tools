...
void print_timestamp(unsigned long long time_us)
{
    time_t tv_sec = time_us /1000000 ; 
    int  tv_msec = (time_us % 1000000)/1000 ;

    struct tm tm_info ; 
    localtime_r(&tv_sec, &tm_info);

    char buffer[26] ; 

    strftime(buffer,26, "%Y-%m-%d %H:%M:%S", &tm_info);
    fprintf(stdout, "%s.%3d  FINISH ONE FRAME\n", buffer,tv_msec);
}
...
int main(int argc, char* argv[])
{
    ...
    int c;
    const char* short_options = "h?d:p:s:f:S:n:b:BD";
